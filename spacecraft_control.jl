...
# ╔═╡ 4938c65a-f087-4c26-8f9f-2c2166518017
using ControlSystems, Plots, PlotlyJS

# ╔═╡ c41d05c8-4f31-460d-b8ab-84252ed6c59c
Plots.plotlyjs();

# ╔═╡ 57a544cf-c29c-49be-9764-f775f4f1f693
md"""
# Plant Model
"""

# ╔═╡ eac166a6-882a-467d-b4fd-0207dcc7ea79
md"""
## Rigid Body Dynamics
The equations that governs spacecraft attitude control are euler's equation for rotational motion

T = Iω̇ + İω + ω×Iω

where T is the torque on the body, I is the inertia 3x3 inertia tensor, and ω is the angular velocity of the body (or body rates as they're often called).

For these examples, we will assume that the inertia is constant (İ = 0) and will ignore the gyroscopic torque (ω×Iω) as part of the linear assumptions. Thus, our equations of motion are simply

T = Iω̇

The transfer function for this plant system is the relationship from a torque (T) input to the acceleration (ω̇) output. Thus our plant is simply
ω̇/T = 1/I

Since we have removed the coupling, this model will treat each of the three body axes independently, thus T, I, and ω are not just 1 dimensional scalars.
"""

# ╔═╡ e6e62a1d-4d73-4a19-b58a-bd587d9cb65d
I = 100.0; # kg

# ╔═╡ 420d15ad-c771-4505-9276-a2d506715ccb
md"""
Typically what we're trying to control is not the torque, but either the rate or attitude. For this example we'll control the attitude, which in a single axis will just be θ. To get θ, we must integrate our ω̇ twice. Thus, our total rigid body plant is 
"""

# ╔═╡ db5d1917-9ef4-4ee9-9d60-ab75980991a1
R = tf([1], [I,0,0]) # = 1/Is^2

# ╔═╡ 0d88774a-44a9-11f0-2b12-579d88494990

	md"""
	## Actuator Dynamics
	For this system we will use a reaction wheel to control the spacecraft. A reaction wheel is simply a DC motor, but the model itself can have varying levels of complexity and fidelity. We will assume our reaction wheel takes a torque command, and just outputs a true torque with some delay modeled as a first order response.	
	"""

# ╔═╡ 9f22a271-7b11-44ad-a537-ba58c266e9e2
τ = 0.1; # arbitrarily chosen

# ╔═╡ 35cf46bd-c8b0-4a78-a29f-f9bb500f2d0e
A = tf([1],[τ,1]) # first order system tf K/(τs + 1)

# ╔═╡ 6466e83e-e176-48df-91bd-f705c8f91056
Plots.plot(step(A,1.0))

# ╔═╡ fb61a064-c21c-4e94-bab2-9438050ddcee
md"""
Reminder for a first order system:
- step response is 63.2% of final value at t = τ
- step response is 70% of final value at t ~ 1.2τ
- bandwidth is 1/τ
"""

# ╔═╡ 873897b3-befd-45c2-be42-b656b865a753
bodeplot(A, hz = true)

# ╔═╡ 24287168-f6d8-482f-9bf2-b7ea5007db72
md"""
## Sensor Dynamics
We'll start by simply having a star tracker that returns the true θ, but with a small delay. We can ignore noise for control stability, though it's important in the time domain, or for stochastic stability of estimators/observers.
"""

# ╔═╡ f48245ef-d84a-49cd-a6d3-c1c7c6cbf6e9
st_delay = 0.1; # 100 ms

# ╔═╡ 19a004b7-f455-4818-bd0f-992dc9a68c82
S = pade(st_delay, 1) # order chosen arbitrarily for now

# ╔═╡ 60b46c6d-36f8-4e01-b18b-21216ba915cd
st_step_response = step(S,1.0);

# ╔═╡ 82cec3d1-7b4c-4016-a154-810b3846a186
Plots.plot(st_step_response)

# ╔═╡ 44ba6431-7f67-419c-af68-343f41221988
md"""
It's interesting to note the difference in the response between the first order actuator model and the 1st order pade approximation of the sensor model. The step response looks somewhat similar, but the frequency response is very different. The first order model has a low pass filtering affect, but the pade approximation maintain the dynamics so that the magnitude is constant regardless of the input frequency. Use pade approximations for pure transport delays, and use the first order model when sluggish physical response is expected due to inertia or other effects. (I dont show the pade frequency response because of errors in the plotting due to quantization while staying at 0db).
"""

# ╔═╡ 61f6eb12-33f6-49c1-b3c2-24e3e4dd488e
md"""
## Flexible Body Dynamics
Flex is modeled as a second order oscillator. The only input we model is the reaction wheel disturbance and the output affects only the star tracker displacement. Without an actual finite-element-model (FEM) from the mechanical team, we can just generically choose parameters here and then scale the output. 
"""

# ╔═╡ cb0e45db-aeb3-4f81-8174-b25ed9575dd1
wn = 1*2*pi; # 1 Hz converted to radians per second

# ╔═╡ 53cbd13c-569e-4b5d-b112-ab08c5938e91
ζ = 0.005; # 1/2 % damping

# ╔═╡ 95ee2eae-881c-4345-a7ec-2a8a2e1899b3
flex_scale = 0.01;

# ╔═╡ 0cd06bcf-58f9-4107-8409-e757dd2dfcf3
F = flex_scale * tf(wn^2,[1, 2*wn*ζ, wn^2]);

# ╔═╡ 8f555219-c690-4c6b-a98e-cf2da6432fd2
Plots.plot(impulse(F))

# ╔═╡ 63078433-561d-4aca-90fa-b00254fd9093
md"""
## Composed Plant Dynamics
The overall open loop plant is composed as 
    
Actuators -->|--> Rigid Body --------+--> Sensor
    		 |                       |
    		 |--> Flexible Body -----|
Or S * (R + F) * A

Note the ordering for the operations goes right to left. For the SISO case this does not matter, since the transfer function inputs and outputs are scalars. However, for the cases with multiple inputs or outputs, the transfer functions are matrics, and the ordering does matter. We follow the proper ordering here to establish convention.

Also, we take the minreal as a matter of practice when composing transfer functions to ensure any pole zero cancelletions occur.
"""

# ╔═╡ bbc24ce1-dee3-47f6-a407-a5184c0e2376
G = minreal(S * (R + F) * A)

# ╔═╡ 9dda8251-0c6c-44d9-82aa-2dc0999d9776
rlocusplot(G)

# ╔═╡ 093d9b9e-df6e-4fc6-843b-189c2365a03c
bodeplot(G, hz = true)

# ╔═╡ 9cca7906-7c51-4c68-ab4a-e8f664073aa6
md"""
# Feedback Control
"""

# ╔═╡ 4225bca7-c7e5-4d94-8985-be61e6e9a133
md"""
We'll start by adding a simple PID controller, K and feeding the star tracker output directly to it as negative unity feedback. The closed loop system is now

Reference(R) --> + --> Controller(K) -> Plant(G) ---> Output(Y)
                  -                               |
                  |_______________________________| 
"""

# ╔═╡ 63e33670-ebfd-4914-8bf5-7bd746eff8b3
md"""
## Ziegler-Nichols method

- Set I and D to 0
- Increase P until the ultimate gain (Ku) causes stable and consistent oscillation
- Tu is the stable oscillation period
- See wikipedia for a table with different methods for using Ku and Tu to solve for Kp, Ki and Kd
https://en.wikipedia.org/wiki/Ziegler-Nichols_method
"""

# ╔═╡ ace280f8-7ff3-4a65-8118-32aac47025f3
Ku = 0.001;

# ╔═╡ 3e6367f6-01e3-4bc9-acbc-b74351c29cfb
L = G * Ku; # remember right to left

# ╔═╡ 384e73d0-1dc9-4378-a7b8-b14fd555c296
sys = feedback(L,1);

# ╔═╡ 190ca2fb-86e9-41ec-b5f7-0adeba30b455
Plots.plot(step(sys,5000))
...
manifest_format = "2.0"
project_hash = "8c24305eae58f647fbbb342ac10de44baddfac36"
...
git-tree-sha1 = "aa14c2b6f1ec735ef4f341cd9bfbeb03b2ffe7ea"
...
git-tree-sha1 = "f094e9386152585ab9b40a3d3b397a20405dcac3"
...
git-tree-sha1 = "5451040479cb5f2494a62c570d407953878dc4ad"
uuid = "99c1a7ee-ab34-5fd5-8076-27c950a045f4"
version = "2.5.1"

[[deps.MatrixPencils]]
deps = ["LinearAlgebra", "Polynomials", "Random"]
git-tree-sha1 = "b792c690a83a065476fdd51a9d78aff7ea1bca1d"
